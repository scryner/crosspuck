...
guest log severity는 registry/env가 아니라 host connection override로 제어합니다. 예:
...
`hook install ok`와 API discovery 세부 로그는 debug level 로그이므로 host가 debug 또는 trace guest severity override를 내렸을 때만 나옵니다.
