...
- bottle root에 `crosspuck-driver-env.reg`를 생성합니다.
...
## 3. Bottle에 Wine override와 환경 변수 등록

설치 후 생성된 registry 파일:

```text
~/Library/Application Support/CrossOver/Bottles/Steam/crosspuck-driver-env.reg
```

CrossOver에서 Steam bottle을 선택한 뒤 Run Command로 `regedit`를 실행하고, 위 `.reg` 파일을 import합니다.

이 단계는 이제 필수는 아니지만 smoke 테스트에서는 권장합니다.

`crosspuck-driver`는 registry/env 값이 없어도 기본적으로 다음 production mode로 시작합니다.
...
등록 후 Steam이 이미 떠 있었다면 반드시 완전히 종료하고 다시 시작합니다. CrossOver가 override나 환경 변수를 바로 반영하지 않으면 bottle 또는 CrossOver 앱을 재시작합니다.
...
- `OK env registry`: env registry 파일이 생성되어 있습니다.
- `OK driver log`: 로그 파일이 있습니다.
- `WARN log marker missing`: 해당 API 경로를 아직 밟지 않았거나, Steam이 DLL을 로드하지 않았거나, env import가 반영되지 않았을 수 있습니다.
...
환경 변수는 CrossOver Steam bottle의 `regedit`에서 `HKCU\Environment` 아래 `CROSSPUCK_*` 값을 삭제하면 됩니다.
